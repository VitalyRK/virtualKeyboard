export class PageContent {
  ...
  getLang() {
    const langValue = localStorage.getItem(this.keyName);
    if (langValue !== null) {
  ...
  setLang(lang) {
    localStorage.setItem(this.keyName, JSON.stringify(lang));
  }

  fillPage(content) {
    content.forEach(({ nodeName, className, innerContent }) => {
  ...
  fillKeyboard(engContent, ruContent) {
    const keyboardChange = this.root.querySelectorAll('.key-letter');
    if (this.getLang() === 'eng') {
      for (let i = 0; i < keyboardChange.length; i += 1) {
        keyboardChange[i].innerHTML = engContent[i];
        if (keyboardChange[i].classList.contains('letter-rus')) keyboardChange[i].classList.add('key-double');
      }
    } else {
      for (let i = 0; i < keyboardChange.length; i += 1) {
        keyboardChange[i].innerHTML = ruContent[i];
